...
class Application:
  ...
  def _merge_notes(self, gcontact, vcard):
    if hasattr(vcard, "note"):
      ...
      if gcontact.content is None or gcontact.content.text != uitem:
        ...
        print "notes added to %s: (%s)" % ( gcontact.title.text, uitem )
  
        return 1
  
    return 0
  
  def _merge_org(self, gcontact, vcard):
  ...
  def merge_object_with_ref(self, gcontact, vcard, rel_map):
  ...
  def update_cards(self, vcards, gd_client):
    query = gdata.contacts.service.ContactsQuery()
    query.max_results = self.options.contact_limit
    ...
    for (name, vcard) in vcards.items():
      ...
      if len(gcontact) < 1:
        ...
        for map_item in self.VOBJECT_VDATA_COMPLEX_REL_MAP:
          self.merge_object_with_ref(gcontact, vcard, map_item)
  
        self._merge_notes(gcontact, vcard)
        self._merge_org(gcontact, vcard)
        ...
        changed = 0
        for map_item in self.VOBJECT_VDATA_COMPLEX_REL_MAP:
          changed += self.merge_object_with_ref(gcontact[0], vcard, map_item)
  
        # organisation and notes
        changed += self._merge_notes(gcontact[0], vcard) \
            + self._merge_org(gcontact[0], vcard)
  ...
  def login(self, username, password):
    ...
    gd_client.source = "none-vcardtogmail-" + __VERSION__
    ...
    try:
      ...
      print >> sys.stderr, "Google has requested you use a captcha to ", \
          "login.\n\nYou'll just have to wait for a while I'm afraid.\n"
      exit(1)
...
def main(argv):
  ...
  gd_client = app.login(options.username, options.password)
  ...
  app.update_cards(vcards, gd_client)
